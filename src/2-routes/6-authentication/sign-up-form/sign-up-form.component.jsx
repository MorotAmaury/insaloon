...
import CustomSelect from "../../../1-components/7-element-select/element-select.component";
import Loader from '../../../1-components/12-loader/loader.component';

import { anneeData, matiereData, prepaData } from "./selectData";
...
const SignUpForm = () => {
  const { setNombrePrompt } = useContext(UserContext)
  const { userMatiere, annee, prepa} = useContext(SelectContext)
  ...
  const successAuth = async () => {
    try {
      ...
      await createUserDocumentFromAuth(user, { nom, prenom, prepa, annee, userMatiere});
      setIsSub(false)
      setNombrePrompt(0)
      resetFormFields();
  ...
  const handleSubmit = async (event) => {
    ...
    if (!userMatiere)
    {
      setErrorMessage("Sélectionnez vos matiere")
      return
    }
    if (!annee) 
    {
      setErrorMessage("Sélectionnez votre année")
      return
    }
    if (!prepa)
    {
      setErrorMessage("Sélectionnez une prépa")
      return 
    }
    ...
    {
      setErrorMessage("Merci d'utiliser une véritable adresse e-mail")
      return
    }
    ...
    <Fragment>
        <div className="sign-up-container">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="double">
                <CustomSelect elementOptions={prepaData} placeholder={'Sélectionnez votre prépa'}  prepa={true}/>
                <CustomSelect elementOptions={anneeData} placeholder={'Sélectionnez votre année'} annee={true}/>
            </div>
            <div className="double">
              <FormInput
                type="email"
                ...
                value={email}
              />
              <CustomSelect elementOptions={matiereData} placeholder={'Sélectionnez vos matières'} matiere={true}/>
            </div>
            <div className="double">
              <FormInput
                type="password"
